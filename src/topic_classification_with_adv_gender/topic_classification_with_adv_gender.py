...
def predict(args,model, corpus, split_name, calculate_loss=False):
    ...
    all_tp_preds,tp_golds = np.array([]),np.array([])
    ovrl_loss,ovrl_tp_loss, step_num = 0.0, 0.0, 0
    with torch.no_grad():
            for step,batch in enumerate(corpus.iter_batches(
                ...
                out,tp_out,lss = model(input_ids=batch_input_ids,
                                       token_type_ids=batch_segment_ids,
                                       attention_mask=batch_attention_masks,
                                       labels = batch_labels,
                                       topic_labels = batch_topic_labels,
                                       alpha = args.alpha)
                ...
                tp_out = tp_out.cpu()
                batch_topic_labels = batch_topic_labels.cpu()
                tp_probs =  torch.nn.functional.softmax(tp_out,dim=1)
                tp_preds = torch.argmax(tp_probs, dim=1)
                all_tp_preds = np.concatenate((all_tp_preds,tp_preds.numpy()))
                tp_golds     = np.concatenate((tp_golds,batch_topic_labels.numpy()))
                if calculate_loss:
                    loss   = torch.nn.functional.cross_entropy(tp_out,batch_topic_labels).mean()
                    ovrl_tp_loss += loss


    return all_preds,golds,all_tp_preds,tp_golds,ovrl_loss/max(1,step_num),ovrl_tp_loss/max(1,step_num)
       ...
       # model = torch.nn.DataParallel(model)
        ...
        for epoch in range(args.epochs):
            ...
            for step,batch in enumerate(corpus.iter_batches(
                ...
                batch_labels = torch.tensor(batch[3]).to(device)
                batch_topic_labels = torch.tensor(batch[4]).to(device)
                logits,tp_logits,loss = model(input_ids=batch_input_ids,
                                              token_type_ids=batch_segment_ids,
                                              attention_mask=batch_attention_masks,
                                              labels = batch_labels,
                                              topic_labels = batch_topic_labels,
            ...
            # Evaluate
            preds,golds,tp_preds,tp_golds,lss,tp_lss = predict(args,model,corpus,'test',calculate_loss=True)
            result = metrics(preds,golds)
            nice_print(result)
            logger.info('')
            result_topic = metrics(tp_preds,tp_golds,target_names=['tech','not_th'])
            nice_print(result_topic,cats=['tech','not_th'])
            ...
            avg_f1 = (float(result_topic['tech']['f1-score'])+float(result_topic['not_th']['f1-score']))/2
            if (float(result_topic['tech']['accuracy']) > 0.4 and 
                float(result_topic['not_th']['accuracy']) > 0.4 and avg_f1 > best_f1):
                saved_any = True
                best_f1 = avg_f1
                ...
                pd.DataFrame({'prediction':preds, # Gender predictions
                              'topic_preds':tp_preds, # Topic predictions
                              'gender_gold':golds,
                              'topic_gold':tp_golds}).to_csv(pred_out_fname,index=False,sep=',')
            ...
            if avg_f1 > best_backup_f1 and saved_any == False:
                ...
                pd.DataFrame({'prediction':preds, # Gender predictions
                              'topic_preds':tp_preds, # Topic predictions
                              'gender_gold':golds,
                              'topic_gold':tp_golds}).to_csv(backup_pred_out_fname,index=False,sep=',')
...
def main():
    ...
    parser.add_argument("--langs",nargs='+',default=['de','es','fr','tr'],help='languages to be considered')
    ...
    parser.add_argument("--alpha",type=float,default=1.0,help='lambda value for gradient reversal layer')
    
    args = parser.parse_args()
    args.langs.sort()
    print(args)
    args.logfname = '{}_{:%Y-%m-%d_%H-%M-%S}'.format(args.model_type,datetime.now())
    fh = logging.FileHandler('./logs/adversarial_training_{}.log'.format(args.logfname))
