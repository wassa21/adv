    ...
    for cv_ix in range(len(corpus.indices)):
       ...
       # model = torch.nn.DataParallel(model)
        ...
        for epoch in range(args.epochs):
            ...
            avg_f1 = (float(result['male']['f1-score'])+float(result['female']['f1-score']))/2
            if (float(result['male']['accuracy']) > 0.4 and 
                float(result['female']['accuracy']) > 0.4 and avg_f1 > best_f1):
                saved_any = True
                best_f1 = avg_f1
