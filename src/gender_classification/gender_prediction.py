...
def nice_print(result):
    ...
    for g in ['male','female','weighted avg','macro avg']:
...
def predict(args,model, corpus, split_name, calculate_loss=False):
    ...
    with torch.no_grad():
            for step,batch in enumerate(corpus.iter_batches(
                ...
                batch_topic_labels = torch.tensor(batch[4])                
                ...
                golds_topic = np.concatenate((golds_topic,batch_topic_labels.numpy()))
                if calculate_loss:
                    loss   = torch.nn.functional.cross_entropy(out,batch_labels).mean()
                    ovrl_loss += loss
                    step_num  += 1                
    return all_preds,golds,ovrl_loss/max(1,step_num),golds_topic
       ...
       # model = torch.nn.DataParallel(model)
        ...
        for epoch in range(args.epochs):
            ...
            for step,batch in enumerate(corpus.iter_batches(
                ...
                batch_labels = torch.tensor(batch[3]).to(device)
                logits,loss = model(input_ids=batch_input_ids,token_type_ids=batch_segment_ids,attention_mask=batch_attention_masks,labels = batch_labels)
                loss = loss.mean()
                ...
                if (step_num) % accumulation_steps == 0:
                    ...
                    model.zero_grad()           
            # Evaluate
            preds,golds,lss,tp_golds = predict(args,model,corpus,'test',calculate_loss=True)
...
def main():
    ...
    parser.add_argument("--model_type",default='GenderPredictor_BERT_LSTM',help='which model to be used')

    args = parser.parse_args()
    args.langs.sort()
    print(args)
    args.logfname = '{}_{:%Y-%m-%d_%H-%M-%S}'.format(args.model_type,datetime.now())
    fh = logging.FileHandler('./logs/gender_prediction_{}.log'.format(args.logfname))
